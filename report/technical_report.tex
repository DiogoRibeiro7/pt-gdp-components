...
Where §3 imposes the regime windows by hand, a two-state Markov-switching regression \cite{hamilton1989} on GDP growth lets the data date them. With switching mean and switching variance it separates a low-growth state, mean -0.448\% per quarter with an expected duration of 4.3 quarters, from a high-growth state, mean 0.506\% and an expected duration of 32.8 quarters. The smoothed probability of the low-growth state exceeds one half in 15 of 119 quarters; laid over the hand-drawn windows it shows where the calendar dating and the data agree and where a downturn the dummies never encoded still registers.
\begin{figure}[H]\centering
\includegraphics[width=\linewidth]{../output/msm_probabilities.png}
\caption{Smoothed probability of the low-growth regime, fixed windows shaded}
\end{figure}


The SUR model is a conditional-mean model; quantile regression \cite{koenkerbassett1978} fits the same design across the conditional distribution of growth and exposes the asymmetry the mean hides. The regime effects fan out sharply: the pandemic coefficient runs from -17.921 pp at the tenth percentile to 18.117 pp at the ninetieth, the collapse-and-rebound signature of a shock the OLS mean averages almost to nothing, while the crisis-and-troika effect ranges from -2.342 pp to -0.556 pp. Where a coefficient is flat across quantiles the mean is a fair summary; where it fans, the lower tail carries risk the mean understates.
\begin{figure}[H]\centering
\includegraphics[width=\linewidth]{../output/quantile_coefficients.png}
\caption{GDP-growth quantile coefficients vs the OLS mean}
\end{figure}


Finally, a one-factor dynamic factor model \cite{stockwatson2011} names the co-movement the SUR layer only registers as a singular residual covariance. A single latent common cycle, with AR(1) dynamics and idiosyncratic AR(1) noise, accounts on average for an R-squared of 0.65 of the component contributions' variation; the strongest loading is on Imports of goods at -0.938. Components with large positive loadings move with the broad demand cycle, those near zero follow their own path --- a compact reading of how synchronised the expenditure side is.
\begin{figure}[H]\centering
\includegraphics[width=\linewidth]{../output/factor_panel.png}
\caption{Common cycle in the expenditure contributions with component loadings}
\end{figure}
